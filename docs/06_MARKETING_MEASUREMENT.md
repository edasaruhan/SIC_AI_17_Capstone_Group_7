...
Status: Operational analytics v1 and deterministic last-touch attribution are implemented;
provider-funded KPI contracts and causal measurement decisions still require Project Lead approval.
...
| Revenue | Recognized order revenue | Gross/net basis, discounts, refunds, tax/shipping treatment, currency, order status | Implemented as `analytics-v1`: discounted placed-order sales and separately posted refunds; tax/shipping/FX excluded |
| Attributed revenue | Revenue linked under a named/versioned attribution method | Evidence class, method, window, exclusions, overlap handling | Deterministic last-touch v1 implemented with a 30-day bound; explicitly noncausal |
...
| Customer value / LTV | Historical or expected customer economic value | Horizon, revenue versus contribution, discounting, censoring, model/method | Historical ledger net value implemented; future LTV not claimed |
...
| Retention | Eligible customers retained under an approved cohort rule | Cohort start, retained event, horizon, reactivation handling | Observed cohort active share implemented; causal retention lift not claimed |
